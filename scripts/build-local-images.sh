...
docker build --pull \
  ...
  -t "$runtime_image" platform/processing/spark

echo "Building $batch_image"
docker build --build-arg RUNTIME_IMAGE="$runtime_image" -t "$batch_image" examples/golden-paths/batch-orders

echo "Building $cdc_image"
docker build --build-arg RUNTIME_IMAGE="$runtime_image" -t "$cdc_image" examples/golden-paths/cdc-orders
...
docker build \
  ...
  -f platform/orchestration/airflow/Dockerfile -t "$airflow_image" .
