...
def _prepare_row_conditions(df: DataFrame, rules: tuple[QualityRule, ...]):
    flagged = df
    ...
    for index, rule in enumerate(rules):
        ...
        if rule.type == "unique":
            ...
            flagged = flagged.withColumn(duplicate_column, F.count(F.lit(1)).over(Window.partitionBy(rule.column)))
...
def _dataset_metric(df: DataFrame, rule: QualityRule, row_count: int) -> RuleMetric:
    if rule.type == "row_count":
        violated = ((rule.minimum is not None and row_count < rule.minimum) or (rule.maximum is not None and row_count > rule.maximum))
    ...
    return RuleMetric(rule.name, rule.type, rule.severity, rule.action, row_count, violations, _status(rule, violations))
...
def evaluate(df: DataFrame, contract: DataContract) -> QualityEvaluation:
    ...
    aggregates = [F.sum(F.when(conditions[rule.name], 1).otherwise(0)).cast("long").alias(f"metric_{index}") for index, rule in enumerate(row_rules)]
    ...
    for index, rule in enumerate(row_rules):
        ...
        metrics.append(RuleMetric(rule.name, rule.type, rule.severity, rule.action, row_count, violations, _status(rule, violations)))
    ...
    error_values = [F.when(conditions[rule.name], F.lit(rule.name)) for rule in quarantine_rules]
    warning_values = [F.when(conditions[rule.name], F.lit(rule.name)) for rule in warning_rules]
    empty_array = F.array().cast("array<string>")
    flagged = flagged.withColumn("_dq_errors", F.filter(F.array(*error_values), lambda value: value.isNotNull()) if error_values else empty_array).withColumn("_dq_warnings", F.filter(F.array(*warning_values), lambda value: value.isNotNull()) if warning_values else empty_array)
    valid = flagged.filter(F.size("_dq_errors") == 0).drop(*temporary_columns, "_dq_errors", "_dq_warnings")
    invalid = flagged.filter(F.size("_dq_errors") > 0).drop(*temporary_columns).withColumn("_quarantined_at", F.current_timestamp())
    return QualityEvaluation(valid=valid, invalid=invalid, metrics=metric_tuple)


def metrics_dataframe(spark: SparkSession, run_id: str, pipeline: str, contract_version: str, metrics: tuple[RuleMetric, ...]) -> DataFrame:
    rows = [(run_id, pipeline, contract_version, m.rule_name, m.rule_type, m.severity, m.action, m.evaluated_rows, m.violation_count, m.status) for m in metrics]
    return spark.createDataFrame(rows, "run_id string, pipeline string, contract_version string, rule_name string, rule_type string, severity string, action string, evaluated_rows long, violation_count long, status string").withColumn("recorded_at", F.current_timestamp())
